...
ACorrCircularFreqAVX_py::ACorrCircularFreqAVX_py(uint64_t N_in, uint64_t size_in)
{
	...
	if(threads > howmany){threads=howmany;}
	...
	public:
		...
		xCorrCircularFreqAVX(py::array_t<double,1> py_in1, py::array_t<double,1> py_in2)
		{
			...
			for(uint64_t i=0;i<threads;i++)
			{
				...
				fftw_execute_dft_r2c(plan,inThreads[i],
								reinterpret_cast<fftw_complex*>(outThreads1[i]));
				std::memcpy(inThreads[i],py_ptr2+i*transferSize,transferSize*sizeof(double));
				fftw_execute_dft_r2c(plan,inThreads[i],
								reinterpret_cast<fftw_complex*>(outThreads2[i]));
				std::memset(inThreads[i],0.0,2*cSize*howmanyPerThread*sizeof(double));
...
class DigitizerXCorrCircularFreqAVX_py
{
	public:
		...
		xCorrCircularFreqAVX(
			py::array_t<DataType,1> py_in1, 
			py::array_t<DataType,1> py_in2,
			double conv,
		   	DataType offset)
		{
